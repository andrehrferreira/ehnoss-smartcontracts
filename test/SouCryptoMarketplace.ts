...
describe("SouCrypto Marketplace", function () {
    ...
    beforeEach(async function () {
        ...
        //Marketplace
        const ContractSouCryptoMarketplaceFactory = await ethers.getContractFactory("SouCryptoMarketplace");
    
        contractSouCryptoMarketplace = await ContractSouCryptoMarketplaceFactory.deploy(
    ...
    it("should mint nft and transfer", async function () {
        ...
        //Open trande
        await contractSouCryptoNft.approve(contractSouCryptoMarketplace.address, 0, { from: owner.address });

        let tx1 = await (await contractSouCryptoMarketplace.openTrade(0, oneEth, { from: owner.address })).wait();
        const transEvent1 = tx1.events?.filter((event) => event.event == "TradeStatusChange")[0].args;

        //Execute
        let tx2 = await (await contractSouCryptoMarketplace.connect(addr3).executeTrade(transEvent1?.ad, { 
            from: addr3.address,
            value: oneEth
        })).wait();

        const transEvent2 = tx2.events?.filter((event) => event.event == "TradeStatusChange")[0].args;

        //Check fees
        expect(await waffle.provider.getBalance(addr1.address)).to.equal(balanceTeamWallet.add(oneEth.mul(1).div(100)));
        expect(await waffle.provider.getBalance(addr2.address)).to.equal(balanceCreatorWallet.add(oneEth.mul(5).div(100)));
    });
});